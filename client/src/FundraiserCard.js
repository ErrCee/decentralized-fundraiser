...
const FundraiserCard = (props) => {
    ...
    const init = async (fundraiser) => {
        try {
            ...
            const isOwner1 = isOwner.toLowerCase()

            if (isOwner1 === accounts[0]) {
                setIsOwner(true)
            }
        }
        catch (error) {
            alert(
                `FundraiserCard.js: Failed to load web3, accounts, or contract. Check console for details.`,
            );
            console.error(error);
        }
    }
            ...
            return (
                <div className="donation-list">
                    ...
                    <Button variant="contained" color="primary">
                        <Link to='/receipts'
                            state={{ fund: fundName, donation: donation.donationAmount, date: donation.date }}
